...
proto.forEachThread = function ( f ) {
    var ret_val;
...
proto.toArray = function ( ) {
    var a = [];
    this.forEachThread(function( it ){
        a.push(it);
    });
    return a;
};
...
proto.filter = function ( f ) {
    return this.mapThread(function( it ){
...
proto.toString = function ( ) {
    ...
    this.body.forEachThread(function( it ){
        buf.push(it, "\n");
    });
    buf.push("})");
    return buf.join("");
...
proto.toString = function ( ) {
    ...
    this.clauses.forEachThread(function( it ){
        buf.push(it, "\n");
    });
    buf.push("}");
    return buf.join("");
...
proto.toString = function ( ) {
    var buf = [ labelsToString.call(this),
                "try ", this.tryBlock, "\n" ];
    this.catchList.forEachThread(function( it ){
        buf.push(it);
    });
    return buf.join("");
};
...
function CfConvert ( func ) {
    ...
    cache.values().forEachThread(function( it ){
        cache2.put(new Identifier(it.id), it);
    });
    var body  = cons(null, nil());
    append = adder(body);
    cacheToList(cache2).forEachThread(function( it ){
...
function check_cyclic ( depends ) {
    ...
    function traverse ( id, path ) {
        ...
        path.forEachThread(function( it ){
    ...
    var result = [];
    depends.keys().forEachThread(function( it ){
        var path = [];
        traverse(it, path);
...
IL.GotoBlock.prototype[Cf] = function ( cache ) {
    ...
    block.scopes    = this.scopes.mapThread(function( it ){ return it[Cf](cache); });
    block.body      = this.body.mapThread(function( it ){ return it[Cf](cache); });
...
IL.CallBlock.prototype[Cf] = function ( cache ) {
    ...
    block.args      = this.args.mapThread(function( it ){ return it[Cf](cache); });
    block.target    = CfTarget(this.target, cache);
    block.exception = CfTarget(this.exception, cache);
    return block;
};
...
ArrayInitializer.prototype[Cf] = function ( cache ) {
    return new ArrayInitializer(this.elems.mapThread(function( it ){
        return it[Cf](cache);
    }));
};
...
ObjectInitializer.prototype[Cf] = function ( cache ) {
    return new ObjectInitializer(this.pairs.mapThread(function( it ){
...
SwitchStatement.prototype[Cs] = function ( follows, ctxt, sttop ) {
    ...
    try {
        ...
        var cond_and_target = this.clauses.reverse().mapThread(function( clause ){
            follows = cons( ctxt.makeGotoBlock(undefinedExp, follows.car), follows );
        ...
        cond_and_target.forEachThread(function( it ){
...
TryFinallyStatement.prototype[Cs] = function ( follows, ctxt, sttop ) {
    ...
    try {
        ...
        ctxt.contBreak.keys().forEachThread(function( label ){
        ...
        ctxt.contContinue.keys().forEachThread(function( label ){
...
function inner_function ( func ) {
    var blocks = func.body.mapThread(function(it){ return it[Cz](); });
...
function var_declaration ( vars ) {
    ...
    var decls = [];
    vars.forEachThread(function( it ){
...
IL.GotoBlock.prototype[Cz] = function ( ) {
    var body = this.body.mapThread(function( it ) {
        return it[Cz]();
    });
    body = cons(null, body);
...
function compile ( src ) {
    ...
    prog = CsConvert(prog);
    prog = CfConvert(prog);
    prog.vars.forEachThread(function( it ){
...
function set_handlers ( args, signal ) {
    ...
    args = args.mapThread(function( arg ){
