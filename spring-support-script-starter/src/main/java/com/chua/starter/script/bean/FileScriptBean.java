 ...
 * @author CH
 */
public class FileScriptBean implements ScriptBean {
    ...
    public FileScriptBean(File source, Watchdog watchdog, Class<?> type) {
        this.source = source;
        this.suffix = FilenameUtils.getExtension(source.getName());
        this.watchdog = Optional.ofNullable(watchdog).orElse(DelegateWatchdog.getInstance());
        this.type = type;
        this.marker = Marker.create(suffix);
        this.watchdog.register(source, this);
        this.initial();
    }
