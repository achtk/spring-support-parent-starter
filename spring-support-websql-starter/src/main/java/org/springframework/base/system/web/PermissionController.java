...
import org.springframework.base.system.web.dto.TempDto;
...
public class PermissionController extends BaseController {
    ...
    public List<Map<String, Object>> allDatabaseList() {
        ...
        try {
            listDb = permissionService.getAllDataBaseById();
        } catch (Exception e) {
    ...
    public Map<String, Object> executeSqlTest(@PathVariable String databaseConfigId, String sql, String databaseName) {
        ...
        if (databaseType.equals("MySql")) {
            if (StringUtils.startsWithAny(sql.toLowerCase(), "select", "show")) {
                ...
                permissionService.saveLog(sql, username, ip);
            }
        }
        if (databaseType.equals("MariaDB")) {
            if (StringUtils.startsWithAny(sql.toLowerCase(), "select", "show")) {
    ...
    public List<Map<String, Object>> selectSearchHistory() {
        List<Map<String, Object>> list = permissionService.selectSearchHistory();
        List<Map<String, Object>> list2 = new ArrayList<>();
    ...
    public Map<String, Object> deleteRows(@RequestBody IdsDto tem, @PathVariable String databaseConfigId)
        ...
        try {
            ...
            permissionService.saveLog("delete from " + databaseName + "." + tableName + " where 1=1 " + condition.toString(), username, ip);
            mess = "删除成功";
            status = "success";
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
        ...
        try {
            ...
            if (updated != null) {
                ...
                if (databaseType.equals("MySql")) {
                    ...
                    permissionService.saveLog("update " + databaseName + "." + tableName + " " + condition.toString(), username, ip);
                }
                if (databaseType.equals("MariaDB")) {
                    permissionService.updateRowsNew(databaseName, tableName, condition, databaseConfigId);
                    permissionService.saveLog("update " + databaseName + "." + tableName + " " + condition.toString(), username, ip);
    ...
    public String dataListToStringForMSSQL(String tableName, Map<String, String> TableColumnType, List<Map<String, Object>> dataList) {
        ...
        for (Map<String, Object> map4 : dataList) {
            ...
            for (Map.Entry<String, Object> entry : map4.entrySet()) {
                ...
                if (entry.getValue() == null) {
                    ...
                    values = values + "'" + entry.getValue() + "',";
                        ...
                        || (TableColumnType.get(key).equals("integer")) || (TableColumnType.get(key).equals("bit")) || (TableColumnType.get(key).equals("NUMBER"))
                        || (TableColumnType.get(key).equals("bigint")) || (TableColumnType.get(key).equals("long")) || (TableColumnType.get(key).equals("float"))
            ...
            sb.append(tempColumnName.substring(0, tempColumnName.length() - 1) + " ) VALUES ( " + values.substring(0, values.length() - 1) + " ); \r\n");
        }
        return sb.toString();
    }
    ...
    public Map<String, Object> dropTable(@RequestBody IdsDto tem, @PathVariable String databaseConfigId)
        ...
        try {
            if ((tableName != null) || (!"".equals(tableName))) {
                ...
                permissionService.saveLog("drop table " + databaseName + "." + tableName, username, ip);
            }
            mess = "删除成功";
            status = "success";
        } catch (Exception e) {
    ...
    public Map<String, Object> dropDatabase(@RequestBody IdsDto tem, @PathVariable String databaseConfigId)
        ...
        try {
            ...
            mess = "删除成功";
            status = "success";
            permissionService.saveLog("drop database  " + databaseName, username, ip);
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
            mess = "删除失败";
            status = "fail";
    ...
    public Map<String, Object> clearTable(@PathVariable String databaseConfigId, @RequestBody IdsDto tem)
        ...
        try {
            ...
            permissionService.saveLog("delete from " + databaseName + "." + tableName, username, ip);
            mess = "删除成功";
            status = "success";
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
    ...
    public Map<String, Object> deleteConfig(@RequestBody IdsDto tem) {
        ...
        String status = "";
        try {
            boolean bl = permissionService.isTheConfigUsed(ids);
            if (bl) {
    ...
    @RequestMapping(value = {"i/dataSynchronizeList"}, method = {RequestMethod.GET})
    @ResponseBody
    public Map<String, Object> dataSynchronizeList() {
        Page<Map<String, Object>> page = getPage(request);
        try {
            page = permissionService.dataSynchronizeList(page);
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
            return getEasyUIData(page);
        }
    ...
    @RequestMapping(value = {"i/editDataSynchronizeForm/{id}"}, method = {RequestMethod.GET})
    public String editDataSynchronizeForm(@PathVariable("id") String id, Model model) {
        Map<String, Object> map = new HashMap<>();
        List<Map<String, Object>> configList = permissionService.getAllConfigList();
        try {
            map = permissionService.getDataSynchronize(id);
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
        }
        model.addAttribute("configList", configList);
    ...
    @RequestMapping(value = {"i/dataSynchronizeLogList/{dataSynchronizeId}"}, method = {RequestMethod.GET})
    @ResponseBody
    public Map<String, Object> dataSynchronizeLogList(@PathVariable String dataSynchronizeId) {
        Page<Map<String, Object>> page = getPage(request);
        try {
            page = permissionService.dataSynchronizeLogList(page, dataSynchronizeId);
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
            return getEasyUIData(page);
        }
    ...
    @RequestMapping(value = {"i/deleteDataSynchronizeLog"}, method = {RequestMethod.POST})
    @ResponseBody
    public Map<String, Object> deleteDataSynchronizeLog(@RequestBody IdsDto tem) {
        String[] ids = tem.getIds();
        String mess = "";
        String status = "";
        try {
            permissionService.deleteDataSynchronizeLog(ids);
            mess = "删除成功";
            status = "success";
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
